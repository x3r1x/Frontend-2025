...
function calc(expression: string): void {
  ...
  function handleOperands(operandsList: string[]): number | string {
    ...
    if (operandsList.length === 1) {
      ...
      const thisExpression: string = operandsList[0]!.substring(
        1,
        operandsList[0]!.length - 1,
      );
    ...
    if (operations.includes(operandsList[0]!)) {
      ...
      const thisExpression = operandsList[0]!.substring(
        1,
        operandsList[0]!.length - 1,
      );
  ...
  function splitWithoutBrackets(expression: string): string[] {
      ...
      if (activeBrackets === 0) {
        if (expression[i]! !== space) {
          if (
            ...
            (expression[i + 1]! === space || expression[i + 1]! === openBracket)
          ) {
            listOfExpressions = [...listOfExpressions, expression[i]!];
            ...
            if (isNumeric(num)) {
              listOfExpressions = [...listOfExpressions, num];
        ...
        if (activeBrackets === 0) {
          listOfExpressions = [...listOfExpressions, bracketExpression];
    ...
    function returnMinIndex(startIndex: number): number {
      ...
      if (
        expression.indexOf(")", startIndex) !== -1 &&
        expression.indexOf(")", startIndex) < minIndex
      ) {
