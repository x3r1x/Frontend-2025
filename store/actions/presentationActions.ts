...
function deleteSlidesFromPresentation(
  presentation: Presentation,
  slideIds: string[],
): Presentation {
...
function changeSlidesOrder(
  ...
  return {
    ...presentation,
    slideCollection: newOrderIds.reduce(
      (newSlidesOrder: Slide[], id: string) => {
        const slide = presentation.slideCollection.find(
          (slide: Slide) => slide.id === id,
        );
