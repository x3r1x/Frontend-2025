...
function testAddSlideToSelection(): "passed" | "failed" {
  ...
  const actionedEntity: Presentation = addSlideToSelection(
    presentation,
    slide3Id,
  );
...
function testAddObjectToSelection(): "passed" | "failed" {
  ...
  const actionedEntity: Presentation = addObjectToSelection(
    presentation,
    slide2obj2id,
  );
...
export function testMaxSelectionActions(): void {
  ...
  console.log(
    "   3. addObjectToSelection: " + testAddObjectToSelection() + "!",
  );
}
